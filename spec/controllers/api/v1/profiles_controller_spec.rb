...
RSpec.describe 'Profile API', type: :request do
  ...
  describe 'PUT /api/v1/profile' do
    ...
    context 'when it is an authenticated user' do
      ...
      it 'preserves existing ui_settings when updating signature settings' do
        agent.update!(ui_settings: { is_contact_sidebar_open: true, theme: 'dark' })
        ...
        agent.reload
        expect(json_response['ui_settings']['signature_position']).to eq('bottom')
        expect(json_response['ui_settings']['is_contact_sidebar_open']).to be(true)
        expect(json_response['ui_settings']['theme']).to eq('dark')
        expect(agent.ui_settings['signature_position']).to eq('bottom')
